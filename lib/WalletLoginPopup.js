...
export default class WalletLoginPopup extends Popup {
    ...
    constructor() {
        ...
        this.main.append(el("section", el("h2", "WalletConnect - Recommendation"), new Button({
            type: ButtonType.Contained,
            icon: el("img", { src: "/images/wallet-icons/walletconnect.svg" }),
            title: "WalletConnect",
            onClick: () => this.selectWallet("walletconnect"),
        })), el("section", el("h2", "Direct Login"), el("p", "These are options you can try when WalletConnect is not working well. Direct Login requires reconnection every time you start the app, so it may be less convenient compared to WalletConnect."), new Button({
            type: ButtonType.Contained,
