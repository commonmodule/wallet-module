...
class WalletConnectionManager extends EventContainer {
    ...
    async getSigner(targetChainName) {
        if (!this.isConnected)
            throw new Error("Not connected");
        let walletAddress = await UniversalWalletConnector.connect(this.connectedWallet);
        ...
        let provider = UniversalWalletConnector.getConnectedProvider(this.connectedWallet);
        if (!provider)
